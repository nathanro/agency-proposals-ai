...
import { ArrowLeft, Send, FileText, DollarSign, Calendar, Mail } from 'lucide-react';
    ...
    try {
      const content = await grokService.generateProposalContent(
        selectedTemplate.name,
        {
          ...
          industry: '', // Se podría agregar campo de industria
        }
      );
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {selectedTemplate && (
            <>
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="px-4 py-3 bg-gray-100 dark:bg-gray-700 rounded-lg text-lg font-bold text-gray-900 dark:text-white">
                      ${(
                        selectedTemplate.price *
                        (1 - (parseFloat(formData.discount_percentage) || 0) / 100)
                      ).toLocaleString()} {selectedTemplate.currency}
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="mt-4 w-full flex items-center justify-center gap-2 px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              {/* Contenido Generado */}
              {(generatedContent.introduction || generatedContent.scope) && (
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                  ...
                  <div className="space-y-6">
                    {generatedContent.introduction && (
                      <div>
                        <h3 className="font-semibold text-gray-900 dark:text-white mb-2">Introducción</h3>
                        <p className="text-gray-600 dark:text-gray-400">{generatedContent.introduction}</p>
                      </div>
                    )}

                    {generatedContent.scope && (
                      <div>
                        <h3 className="font-semibold text-gray-900 dark:text-white mb-2">Alcance</h3>
                        <p className="text-gray-600 dark:text-gray-400">{generatedContent.scope}</p>
                      </div>
                    )}
                    ...
                    {generatedContent.timeline && (
                      <div>
                        ...
                        <p className="text-gray-600 dark:text-gray-400">{generatedContent.timeline}</p>
                      </div>
                    )}

                    {generatedContent.investment && (
                      <div>
                        <h3 className="font-semibold text-gray-900 dark:text-white mb-2">Inversión</h3>
                        <p className="text-gray-600 dark:text-gray-400">{generatedContent.investment}</p>
                      </div>
                    )}
                  </div>
                </div>
